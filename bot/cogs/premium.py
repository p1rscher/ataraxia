...
class PremiumCog(commands.Cog):
    ...
    @staticmethod
    def _can_manage_server_profile(interaction: discord.Interaction) -> bool:
    ...
    async def set_server_profile(
        ...
        if not (nickname or bio or avatar or banner):
            await ctx.response.send_message("❌ Provide at least one field to update.", ephemeral=True)
            return

        if not self._can_manage_server_profile(ctx):
            await ctx.response.send_message("❌ You need **Manage Server** to use this.", ephemeral=True)
            return
        ...
        reason = f"Requested by {ctx.user} ({ctx.user.id})"
        ...
        await ctx.followup.send("✅ Updated guild-specific bot profile.", ephemeral=True)
    ...
    async def reset_server_profile(
        ...
        if not payload:
            await ctx.response.send_message("❌ Select at least one field to reset.", ephemeral=True)
            return
        ...
        await ctx.followup.send("✅ Reset selected guild-specific bot profile fields.", ephemeral=True)

    @premium_group.command(name="view_server_profile", description="View this bot's guild-specific profile")
    @app_commands.default_permissions(manage_guild=True, administrator=True)
    async def view_server_profile(self, ctx: discord.Interaction):
        if not ctx.guild:
            await ctx.response.send_message("❌ This command can only be used in a server.", ephemeral=True)
            return
        ...
        try:
            if not self.bot.user:
                await ctx.followup.send("❌ Bot user is not ready yet.", ephemeral=True)
                return
