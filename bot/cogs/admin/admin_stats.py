...
class AdminStatsCog(commands.Cog):
    ...
    async def admin_stats(self, ctx: commands.Context):
        ...
        try:
            ...
            total_commands = await self.bot.db.get_total_commands()
            daily_active_users = await self.bot.db.get_daily_active_users()
            command_stats_7d = await self.bot.db.get_command_stats(7)
            command_stats_1d = await self.bot.db.get_command_stats(1)
