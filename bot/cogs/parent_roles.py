...
class ParentRolesCog(commands.Cog):
    ...
    parentrole_group = app_commands.Group(name="parentrole", description="Manage parent role system")
    ...
    async def parent_role_create(
        ...
        if color:
            try:
                ...
                await ctx.response.send_message("❌ Invalid color format! Use hex format like #ff0000", ephemeral=True)
                return
        ...
        try:
            ...
            role = await ctx.guild.create_role(
                ...
                mentionable=False,
                reason=f"Parent role created by {ctx.user}"
            )
            
            # Add to database
            await db.add_parent_role(ctx.guild_id, role.id)
            ...
            logger.info(f"Created parent role '{name}' (ID: {role.id}) in guild {ctx.guild_id}")
            
        except discord.Forbidden:
            await ctx.response.send_message("❌ I don't have permission to create roles!", ephemeral=True)
        except Exception as e:
            await ctx.response.send_message(f"❌ Error creating role: {e}", ephemeral=True)
            logger.error(f"Error creating parent role in guild {ctx.guild_id}: {e}")
    ...
    async def parent_role_add_child(
        ...
        await db.add_child_to_parent(ctx.guild_id, parent_role.id, child_role.id)
        ...
        logger.info(f"Added child role {child_role.name} to parent {parent_role.name} in guild {ctx.guild_id}")
    ...
    async def parent_role_remove_child(
        ...
        await db.remove_child_from_parent(ctx.guild_id, parent_role.id, child_role.id)
        
        await ctx.response.send_message(
            f"✅ Removed {child_role.mention} from parent {parent_role.mention}",
            ephemeral=True
        )
        logger.info(f"Removed child role {child_role.name} from parent {parent_role.name} in guild {ctx.guild_id}")
    ...
    @app_commands.default_permissions(administrator=True)
    @app_commands.guild_only()
    async def parent_role_delete(self, ctx: discord.Interaction, parent_role: discord.Role):
        ...
        await db.remove_parent_role(ctx.guild_id, parent_role.id)
        
        # Optionally delete the role itself
        try:
            await parent_role.delete(reason=f"Parent role deleted by {ctx.user}")
            await ctx.response.send_message(
                f"✅ Deleted parent role and its configuration",
                ephemeral=True
            )
            logger.info(f"Deleted parent role {parent_role.name} (ID: {parent_role.id}) from guild {ctx.guild_id}")
        except discord.Forbidden:
            await ctx.response.send_message(
    ...
    @app_commands.default_permissions(administrator=True)
    @app_commands.guild_only()
    async def parent_role_list(self, ctx: discord.Interaction):
        """List all parent role configurations"""
        parent_configs = await db.get_all_parent_roles(ctx.guild_id)
