...
class CleanupChannelCog(commands.Cog):
    ...
    @commands.hybrid_group(name="channel", description="Cleanup messages in a channel")
    async def cleanup_group(self, ctx: commands.Context):
        pass
    ...
    async def clear_channel(
        ...
        deleted = 0
        status_msg = await ctx.send(f"Deleting {total} messages...", ephemeral=True)
        ...
        for i in range(0, len(messages_to_delete), batch_size):
            ...
            if recent_batch:
                try:
                    ...
                    await ctx.send(f"Error deleting messages in bulk: {e}", ephemeral=True)
