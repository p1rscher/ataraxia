...
class SayCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        # Removed db to avoid import issues if not needed here
    ...
    async def _send_say_log(
        ...
        log_channel_id = await db.get_say_log_channel_id(ctx.guild.id)
        ...
        if target_channel is None:
            try:
                ...
                logger.warning("Configured /say log channel %s could not be fetched in guild %s", log_channel_id, ctx.guild.id)
                return
        ...
        try:
            ...
            logger.exception("Failed to send /say usage log in guild %s", ctx.guild.id)

    @commands.hybrid_command(name="say", description="Responds with a message or attachment")
    @app_commands.describe(message_id="Optional: ID of the message to reply to", attachment="Optional: Attachment to send", message="The message to say")
    @commands.guild_only()
    async def say(self, ctx: commands.Context, message_id: Optional[str] = None, attachment: Optional[discord.Attachment] = None, *, message: Optional[str] = None):
        assert ctx.guild is not None
        ...
        if not message and not attachment:
            await ctx.send("❌ You must provide either a message or an attachment!", ephemeral=True)
            return
        ...
        if message_id:
            try:
                ...
                await ctx.send("❌ Invalid Message ID provided. It must be a number.", ephemeral=True)
                return
        ...
        if attachment:
            try:
                ...
                await ctx.send("❌ Failed to process the attachment.", ephemeral=True)
                return
        ...
        try:
            ...
            # await self._send_say_log(ctx, sent_message, message, message_id, attachment) # Requires ctx.author logic update
            await ctx.send("✅ Message sent silently!", ephemeral=True)
        except discord.HTTPException:
            # Fallback if the message ID was not found
            if ref:
                await ctx.send("❌ Failed to reply. Is the Message ID correct and in this channel?", ephemeral=True)
            else:
                await ctx.send("❌ Failed to send message.", ephemeral=True)
