...
class BumpReminder(commands.Cog):
    ...
    bump_group = app_commands.Group(name="bump", description="Manage bump reminder settings")
    ...
    @app_commands.guild_only()
    async def bump_disable(self, ctx: discord.Interaction):
        # Check admin permissions
        if not ctx.user.guild_permissions.administrator:
            await ctx.response.send_message("❌ You need administrator permissions for this command!", ephemeral=True)
            return
        ...
        await ctx.response.send_message("✅ Bump reminder disabled!", ephemeral=True)

    @bump_group.command(name="status", description="Show the status of the bump reminder system")
    @app_commands.guild_only()
    async def bump_status(self, ctx: discord.Interaction):
        settings = await db.get_bump_settings(ctx.guild_id)
        
        if not settings or not settings['enabled']:
            await ctx.response.send_message("❌ Bump reminder is not enabled!", ephemeral=True)
            return
