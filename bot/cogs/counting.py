...
class CountingCog(commands.Cog):
    ...
    counting_group = app_commands.Group(name="counting", description="Counting channel management")
    ...
    async def counting_setup(self, interaction: discord.Interaction, channel: discord.TextChannel):
        ...
        try:
            await db.set_counting_channel(interaction.guild_id, channel.id)
            ...
            await interaction.response.send_message("❌ An error occurred while setting up the counting channel.", ephemeral=True)
    ...
    async def counting_remove(self, interaction: discord.Interaction):
        ...
        try:
            await db.remove_counting_channel(interaction.guild_id)
            ...
            await interaction.response.send_message("❌ An error occurred while removing the counting channel.", ephemeral=True)

    @counting_group.command(name="status")
    async def counting_status(self, interaction: discord.Interaction):
        """Check the current counting status"""
        try:
            settings = await db.get_counting_settings(interaction.guild_id)
            
            if not settings:
                await interaction.response.send_message("❌ No counting channel has been set up yet.", ephemeral=True)
                return
            ...
            await interaction.response.send_message("❌ An error occurred while checking the counting status.", ephemeral=True)
    ...
    async def counting_reset(self, interaction: discord.Interaction):
        ...
        try:
            await db.reset_counting(interaction.guild_id)
            ...
            await interaction.response.send_message("❌ An error occurred while resetting the counting.", ephemeral=True)
    ...
    async def counting_leaderboard(self, interaction: discord.Interaction):
        ...
        try:
            leaderboard = await db.get_counting_leaderboard(interaction.guild_id, limit=10)
            
            if not leaderboard:
                await interaction.response.send_message("❌ No counting data available yet.", ephemeral=True)
                return
