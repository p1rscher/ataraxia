...
class CustomRoleCog(commands.GroupCog, group_name="customrole"):
    ...
    @commands.hybrid_command(name="setup", description="Setup the anchor role and allowed roles for Custom Roles")
    @commands.has_permissions(administrator=True)
    async def setup_cmd(self, interaction: commands.Context, anchor_role: discord.Role):
        # Save anchor role initially; allowed_roles empty for now until they pick
        await db.set_custom_role_setup(interaction.guild.id, anchor_role.id, [])
    ...
    @commands.hybrid_command(name="toggle", description="Toggle Custom Roles on or off globally")
    @commands.has_permissions(administrator=True)
    async def toggle_cmd(self, interaction: commands.Context):
        settings = await db.get_custom_role_settings(interaction.guild.id)
        new_state = not settings['enabled']
        await db.toggle_custom_role(interaction.guild.id, new_state)
        await interaction.send(f"✅ Custom Roles are now **{'Enabled' if new_state else 'Disabled'}**.", ephemeral=True)

    @commands.hybrid_command(name="manage", description="Create or edit your custom role")
    @app_commands.describe(icon="Optional: Custom role icon image (Server Boost Lvl 2 required)")
    async def manage_cmd(self, interaction: commands.Context, icon: discord.Attachment = None):
        settings = await db.get_custom_role_settings(interaction.guild.id)
        
        if not settings['enabled']:
            return await interaction.send("❌ Custom Roles are currently disabled on this server.", ephemeral=True)
            
        allowed = settings['allowed_roles']
        if allowed:
            user_role_ids = [r.id for r in interaction.author.roles]
            if not any(r_id in allowed for r_id in user_role_ids):
                return await interaction.send("❌ You do not have permission to create a custom role.", ephemeral=True)
        else:
             return await interaction.send("❌ Administrators have not configured any allowed roles yet.", ephemeral=True)
             
        # Fetch existing role if it exists
        existing_role_id = await db.get_user_custom_role(interaction.author.id, interaction.guild.id)
        ...
        class CustomRoleModal(discord.ui.Modal, title='Manage Custom Role'):
            ...
            async def on_submit(self, inter: commands.Context):
                ...
                try:
                    ...
                    return await inter.send("❌ Invalid Hex Color provided. Use format `#RRGGBB`.", ephemeral=True)
                ...
                if existing_role:
                    try:
                        ...
                        if existing_role not in interaction.author.roles:
                            await interaction.author.add_roles(existing_role)
                        ...
                        if icon_bytes:
                            ...
                            return await inter.send(f"❌ Failed to edit role: {e}", ephemeral=True)
                else:
                    try:
                        ...
                        await db.set_user_custom_role(interaction.author.id, interaction.guild.id, new_role.id)
                        ...
                        if icon_bytes:
                            ...
                            return await inter.send(f"❌ Failed to create role: {e}", ephemeral=True)

                await inter.send(success_msg, ephemeral=True)
